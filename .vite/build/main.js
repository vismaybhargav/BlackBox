...
function installAppMenu(win) {
  ...
  const getWin = () => win ?? require$$3$1.BrowserWindow.getFocusedWindow() ?? void 0;
  const template = [
    ...isMac ? [{ role: "appMenu" }] : [],
    {
      label: "File",
      submenu: [
        {
          ...
          click: async () => {
            const bw = getWin();
            const { canceled, filePaths } = await require$$3$1.dialog.showOpenDialog(bw, {
              properties: ["openFile"],
              filters: [{
                name: "CSV Files",
                extensions: ["csv"]
            ...
            win.webContents.send("csv-opened", fs__namespace.readFileSync(filePaths[0], "utf-8"));
          }
        }
      ]
    },
      ...
      submenu: [
        {
          ...
          click: () => {
            const bw = getWin();
            bw == null ? void 0 : bw.webContents.toggleDevTools();
          }
        }
      ]
    }
  ];
  require$$3$1.Menu.setApplicationMenu(require$$3$1.Menu.buildFromTemplate(template));
}
if (started) {
